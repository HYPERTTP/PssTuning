...
clc
close all
bestOrganism =[2.2463    0.2000    0.1500];
Kpss=bestOrganism(1);
T1=bestOrganism(2);
T2=bestOrganism(3);
T=10;
Re=0;
Xe=0.5;  % change the variables 
...
%%
    ...
    [Va,D1]=eig(Apss1);
    diag(D1)  % Eigenvalues without PSS
    WT=inv(Va);W=(WT)';
    WT=inv(Va);W=(WT)';
    for k=1:4
        for l=1:4
    ...
    npf=abs(pfr*diag(1./max(abs(pfr))))  % participation factor without PSS
%%
...
display('with pss')
    ...
    npf1=abs(pfr1*diag(1./max(abs(pfr1)))) % participation factor with PSS
%% PLOT
...
ylabel('{\Delta}{\delta}')
hold on
PSS= 0; D=20;% Simulation with PSS
simopt = simset('solver','ode45','SrcWorkspace','Current','DstWorkspace','Current');  % Initialize sim options
[tout xout yout]=sim('SMIB_sim',[0 Time],simopt);  %#ok
